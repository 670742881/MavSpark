...
object OnLineTheTop3ItemForEachCategory2DB {
  def main(args: Array[String]): Unit = {
    ...
    val topics = Set("test")
   val userClickLogsDStream: ReceiverInputDStream[String] = ssc.socketTextStream("bigdata.server1",9999)
   /// val userClickLogsDStream = KafkaUtils
    ...
    val formattedUserClickDstream: DStream[(String, Int)] = userClickLogsDStream
      .filter(line => line.nonEmpty)
      .map(line => line.split("\t"))
      .filter(arr => arr.length == 3)
      .map(arr => (arr(2)+"_"+arr(1),1))
   ...
   val a: Unit =categoryUserClickLogDstream.foreachRDD(rdd =>{
      if(rdd.isEmpty()){
          ...
          if(partitionOfRecords.isEmpty){
           ...
           var connection:Connection = null
            try{
              ...
              val sql = "insert into demos.categoryItem (category,item,click_count)  values(?,?,?)"
