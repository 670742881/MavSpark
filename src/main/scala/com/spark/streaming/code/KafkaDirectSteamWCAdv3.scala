package com.spark.streaming.code
...
object KafkaDirectSteamWCAdv3 {
  def main(args: Array[String]): Unit = {
   val conf = new SparkConf()
      .setAppName("Spark Streaming WordCount")
      .setIfMissing("spark.master","local[2]")
      .set("spark.streamipark.streaming.backpressure.enabled","true")  //起用反压机制
      .set("spark.streaming.kafka.maxRatePerPartition","200")  //限制没秒从每个kafka的topic分区最大读取的message

//如果是打成jar包运行，bin/spark-submit   --conf  spark.streamipark.streaming.backpressure.enabled=true   --conf spark.streaming.kafka.maxRatePerPartition=200
    ...
    /**
      * 基于直接方式的优化：
      *   1.反压（back） 主要的方式式用来限制每秒钟读取的数据量，比如当前产生的数据量是100万，通过反压之后，可以限制为每秒10万
      *   2.动态资源分配
      */
    ...
    val fromOffsets =Map[TopicAndPartition,Long](
      TopicAndPartition("test",0) -> 0,
      TopicAndPartition("test",1) -> 0,
      TopicAndPartition("test",2) -> 0,
      TopicAndPartition("test",3) -> 0
